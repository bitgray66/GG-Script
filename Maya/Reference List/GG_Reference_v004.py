...
projPath = cmds.workspace(q=True, dir=True) + "Data_from_" + scene_name + ".txt"
print("projPath: " + projPath)
f = open(projPath, 'w')
f.write('{0:*^50}'.format('Risultato') + '\n')
for asset in referenced_assets:
    f.write("Namespace: " + asset['namespace'] + '\n')
    f.write("Reference Node: " + asset['reference_node'] + '\n')
    f.write("File Path: " + asset['file_path'] + '\n')
    #f.write("Nodes in Reference: " + str(asset['nodes_in_reference']) + '\n')
    f.write("-" * 30 + '\n')
f.close()
